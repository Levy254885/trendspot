...
export function MainNav() {
  ...
  return (
    <nav className="border-b border-border bg-white" aria-label="Main">
      <div className="mx-auto flex max-w-[1040px] items-center justify-between px-5 sm:px-10 lg:px-8">
        <div className="hidden items-center justify-center gap-0 overflow-x-auto md:flex md:w-full">
          {NAV_ITEMS.map((item) => (
            <Link
              ...
              href={item.href}
              className={cn(
                "whitespace-nowrap border-b-2 px-3 py-[11px] text-[11.5px] font-bold uppercase tracking-[0.1em] transition-colors lg:px-5",
                isActive(item.href)
                  ? "border-red text-red"
