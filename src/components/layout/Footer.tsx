  ...
  return (
    <footer className="bg-black pb-6 pt-10">
      <div className="mx-auto max-w-[1180px] px-4 sm:px-6">
        ...
        <div className="mb-7 mt-1.5 text-[11px] uppercase tracking-[0.12em] text-white/30">
          {SITE_TAGLINE}
        </div>
        <div className="mb-7 h-px bg-white/8" />
        <div className="mb-7 grid grid-cols-2 gap-6 sm:grid-cols-3 md:grid-cols-5 md:gap-7">
              ...
              <ul className="flex flex-col gap-2">
                {section.links.map((link) => (
                  <li key={link.href + link.label}>
                    <Link href={link.href} className="text-[12.5px] text-white/45 transition-colors hover:text-white">
                      {link.label}
                    </Link>
                  </li>
                ))}
